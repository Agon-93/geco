...
function supaLeggi(codice, callback) {
    ...
    Promise.all([
        fetch(SUPA_URL + '/rest/v1/sync_data?codice=eq.' + encodeURIComponent(codice) + '&limit=1', { headers: h, cache: 'no-store' })
            .then(function(r){ return r.json(); }),
        fetch(SUPA_URL + '/rest/v1/sync_pasti?codice=eq.' + encodeURIComponent(codice), { headers: h, cache: 'no-store' })
            .then(function(r){ return r.json(); })
